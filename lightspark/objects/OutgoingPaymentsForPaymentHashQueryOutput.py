...
class OutgoingPaymentsForPaymentHashQueryOutput():
    ...
    def to_json(self) -> Mapping[str, Any]:
        return {
            "outgoing_payments_for_payment_hash_query_output_payments": [e.to_json() for e in self.payments],
...
def from_json(requester: Requester, obj: Mapping[str, Any]) -> OutgoingPaymentsForPaymentHashQueryOutput:
    return OutgoingPaymentsForPaymentHashQueryOutput(
        requester=requester,        payments=list(map(
 # pylint: disable=unnecessary-lambda 
 lambda e: OutgoingPayment_from_json(requester, e), obj["outgoing_payments_for_payment_hash_query_output_payments"])),

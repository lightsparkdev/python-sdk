...
class SignMessagesOutput():
    ...
    def to_json(self) -> Mapping[str, Any]:
        return {
            "sign_messages_output_signed_payloads": [e.to_json() for e in self.signed_payloads],
...
def from_json(requester: Requester, obj: Mapping[str, Any]) -> SignMessagesOutput:
    return SignMessagesOutput(
        requester=requester,        signed_payloads=list(map(
 # pylint: disable=unnecessary-lambda 
 lambda e: SignablePayload_from_json(requester, e), obj["sign_messages_output_signed_payloads"])),
